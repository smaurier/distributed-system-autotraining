// =============================================================================
// Lab 06 — Message Queues (Solution)
// =============================================================================
...
class ConsumerGroupQueue<T> {
  ...
  addConsumer(groupId: string, consumer: (message: T) => void): void {
    if (!this.groups.has(groupId)) {
      this.groups.set(groupId, { groupId, consumers: [], nextIndex: 0 });
    }
    this.groups.get(groupId)!.consumers.push(consumer);
  }

  publish(message: T): void {
    for (const [groupId, group] of this.groups) {
      if (group.consumers.length === 0) continue;
      const consumerId = group.nextIndex % group.consumers.length;
      group.consumers[consumerId](message);
      this.messageLog.push({ groupId, message, consumerId });
      group.nextIndex = (group.nextIndex + 1) % group.consumers.length;
    }
  }
...
class DeadLetterQueue<T> {
  ...
  reprocessDeadLetters(handler: (msg: T) => void): number {
    let reprocessed = 0;
    const remaining: DLQMessage<T>[] = [];
    for (const dl of this.deadLetters) {
      try {
        handler(dl.originalMessage);
        this.processed.push(dl.originalMessage);
        reprocessed++;
      } catch {
        remaining.push(dl);
      }
...
class OrderedQueue<T> {
  ...
  receive(message: SequencedMessage<T>): T[] {
    // Ignore duplicates (already delivered or already in buffer)
    if (message.sequenceNumber < this.nextExpected) {
      return [];
    }

    if (message.sequenceNumber === this.nextExpected) {
      // Deliver this message and any buffered consecutive messages
      const result: T[] = [];
      result.push(message.payload);
      this.delivered.push(message.payload);
      this.nextExpected++;
      ...
      this.buffer.set(message.sequenceNumber, message.payload);
      return [];
    }
  }
  ...
  enqueue(item: T): boolean {
    ...
    switch (this.strategy) {
      ...
      case 'drop-oldest': {
        ...
        this.dropped.push(oldest);
        this.items.push(item);
        return true;
      }
      case 'reject':
        this.rejected.push(item);
        throw new Error(`Queue full: rejected item`);
    }
  }
